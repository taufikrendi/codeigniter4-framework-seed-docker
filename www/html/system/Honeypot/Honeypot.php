...
class Honeypot
{
    ...
    public function __construct(HoneypotConfig $config)
    {
        ...
        if (empty($this->config->container) || strpos($this->config->container, '{templates}') === false) {
            $this->config->container = '<div style="display:none">{templates}</div>';
        }
    ...
    protected function prepareTemplate(string $template): string
    {
        ...
        if ($this->config->hidden) {
            $template = str_ireplace('{templates}', $template, $this->config->container);
        }
